@extends('layouts.app')
@section('title', 'Sampah Surat Masuk')
@section('content')

<div class="d-flex flex-wrap align-items-end justify-content-between gap-3 mb-4">
    <div>
        <p class="text-muted mb-1" style="font-size:.82rem">Persuratan</p>
        <h2 class="mb-1" style="font-size:1.5rem">Sampah Surat Masuk</h2>
...
<ul class="nav mb-4" style="border-bottom:1px solid var(--border);">
    <li class="nav-item">
        <a class="nav-link px-3 py-2" href="{{ route('surat-masuk.index') }}"
           style="color:var(--ink-muted);font-weight:600;">
            Data Aktif
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link active px-3 py-2" href="{{ route('surat-masuk.trashed') }}"
            ...
            <tbody>
                @forelse($suratMasuk as $s)
                    <tr>
                        ...
                        <td style="font-size:.85rem">{{ $s->asal_instansi }}</td>
                        ...
                        <td class="pe-3 text-end">
                            <div class="d-inline-flex gap-2 justify-content-end">
                                <form action="{{ route('surat-masuk.restore', $s->uuid) }}" method="POST"
                                      onsubmit="return confirm('Pulihkan surat masuk ini?')">
                                    @csrf @method('PUT')
                                ...
                                @if(in_array(Auth::user()->role, ['admin_tu', 'super_admin'], true))
                                    <form action="{{ route('surat-masuk.forceDelete', $s->uuid) }}" method="POST"
                                          onsubmit="return confirm('Hapus PERMANEN surat masuk ini? Data tidak bisa dikembalikan lagi.')">
                                        @csrf @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted py-5" style="font-size:.85rem">
