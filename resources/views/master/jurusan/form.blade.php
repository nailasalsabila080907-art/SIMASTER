...
@section('title', $jurusan->exists ? 'Ubah Jurusan' : 'Tambah Jurusan')

@section('content')

    <div class="d-flex align-items-center gap-2 mb-4">
        <a href="{{ route('jurusan.index') }}" class="btn-icon-ghost">
            <i class="bi bi-arrow-left"></i>
        </a>
        <div>
            <h5 class="mb-0" style="color:var(--ink)">
                {{ $jurusan->exists ? 'Ubah Jurusan' : 'Tambah Jurusan' }}
            </h5>
    ...
    <div class="card" style="max-width:760px;">
        <div class="card-body p-4">
            ...
            <form method="POST" action="{{ $jurusan->exists ? route('jurusan.update', $jurusan) : route('jurusan.store') }}">
                @csrf
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select" style="border-radius:10px;border-color:var(--border);font-size:.87rem;">
                            <option value="aktif" {{ old('status', $jurusan->status ?: 'aktif') === 'aktif' ? 'selected' : '' }}>Aktif</option>
                ...
                <div class="d-flex gap-2 mt-4 pt-3" style="border-top:1px solid var(--border);">
                    ...
                    <a href="{{ route('jurusan.index') }}" class="btn" style="border:1px solid var(--border);border-radius:10px;font-weight:600;font-size:.87rem;padding:.6rem 1.4rem;color:var(--ink);">
                        Batal
                    </a>
                </div>
            </form>
