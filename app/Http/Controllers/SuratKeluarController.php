...
class SuratKeluarController extends Controller
{
    ...
    public function edit(SuratKeluar $suratKeluar)
    {
        ...
        abort_unless(in_array($suratKeluar->status, ['draft', 'ditolak'], true), 422, 'Surat hanya bisa diedit saat masih draft atau ditolak.');
    ...
    {
        $template = TemplateSurat::with('variabel')->findOrFail($request->input('id_template'));
        $this->validasiFormSurat($request, $template);

        $surat = SuratKeluar::create($this->dataSurat($request, $template));
    ...
    {
        $this->pastikanPemilikAtauAdmin($suratKeluar);
        abort_unless(in_array($suratKeluar->status, ['draft', 'ditolak'], true), 422, 'Surat tidak dapat diubah pada status saat ini.');

        $template = TemplateSurat::with('variabel')->findOrFail($request->input('id_template'));
        $this->validasiFormSurat($request, $template);

        $suratKeluar->update($this->dataSurat($request, $template));
        $suratKeluar->approval()->delete();
        $suratKeluar->update(['status' => 'draft', 'nomor_surat' => null, 'isi_surat' => null]);

        LogAktivitas::catat('ubah_data', 'Surat Keluar', "Memperbarui draft surat: {$suratKeluar->perihal}");

        return redirect()->route('surat-keluar.show', $suratKeluar)->with('sukses', 'Draft surat berhasil diperbarui.');
    }
    ...
    {
        $this->pastikanPemilikAtauAdmin($suratKeluar);
        abort_unless(in_array($suratKeluar->status, ['draft', 'ditolak'], true), 422, 'Surat tidak dapat diajukan pada status saat ini.');
        ...
        $approverUsers = \App\Models\User::with('pegawai.jabatan')
            ->where('status', 'aktif')
            ->whereIn('role', ['admin_tu', 'super_admin', 'kepala_sekolah'])
            ->whereHas('pegawai', function ($query) use ($pembuatPegawaiId) {
                $query->where('status', 'aktif')
                    ->where('id_pegawai', '!=', $pembuatPegawaiId)
                    ->whereHas('jabatan', fn ($q) => $q->where('level_jabatan', '>=', 2));
            })
            ->get()
            ->sortBy(fn ($user) => $user->pegawai?->jabatan?->level_jabatan ?? 999)
            ->groupBy(fn ($user) => $user->pegawai?->jabatan?->level_jabatan ?? 999)
            ->map(fn ($users) => $users->first())
            ->values();

        if ($approverUsers->isEmpty()) {
            return back()->with('gagal', 'Belum ada approver aktif dengan level jabatan minimal 2. Isi Master Pegawai dan pastikan akun Kepala TU/Kepala Sekolah aktif.');
        }

        DB::transaction(function () use ($suratKeluar, $approverUsers) {
            $suratKeluar->approval()->delete();

            foreach ($approverUsers as $index => $user) {
                $approval = ApprovalSuratKeluar::create([
    ...
    protected function form(Request $request, ?SuratKeluar $suratKeluar = null)
    {
        ...
        $templateList = $kategoriId
            ? TemplateSurat::where('id_kategori', $kategoriId)->where('is_active', true)->orderBy('nama_template')->get()
            : collect();

        $template = $templateId
            ? TemplateSurat::with('variabel')->find($templateId)
            : null;
    ...
    protected function validasiFormSurat(Request $request, TemplateSurat $template): void
    {
        $request->validate([
        ...
        foreach ($template->variabel as $var) {
            $rules = $var->wajib ? ['required'] : ['nullable'];
            $rules[] = match ($var->tipe_input) {
                'date' => 'date',
                'number' => 'numeric',
                default => 'string',
            };
            $request->validate(['variabel_'.$var->id_variabel => $rules]);
        }
    }

    protected function dataSurat(Request $request, TemplateSurat $template): array
    {
        $dataVariabel = [];
        foreach ($template->variabel as $var) {
        ...
        return [
            ...
            'id_klasifikasi' => KlasifikasiArsip::firstOrFail()->id_klasifikasi,
            'id_unit_pembuat' => $pegawai?->id_unit ?? abort(422, 'Akun Anda belum memiliki unit kerja.'),
    ...
    protected function pastikanPemilikAtauAdmin(SuratKeluar $suratKeluar): void
    {
        abort_unless(
            $suratKeluar->dibuat_oleh === Auth::id() || in_array(Auth::user()->role, ['admin_tu', 'super_admin'], true),
            403,
            'Anda tidak memiliki akses ke surat ini.'
        );
    }

    protected function pastikanBolehLihat(SuratKeluar $suratKeluar): void
    {
        if (in_array(Auth::user()->role, ['admin_tu', 'super_admin', 'kepala_sekolah'], true)) {
            return;
        }

        abort_unless($suratKeluar->dibuat_oleh === Auth::id(), 403, 'Anda tidak memiliki akses ke surat ini.');
    }
}
